...
type Props = {
    filters: Filters;
    onChange: (field: string) => string
}

export const FiltersForm = ({ filters, onChange }: Props) => {
    const {data: space, isLoading, isError} = useCurrentSpace()
    // TODO if error && is Loading
    return (
        space?.data ? 
        <div className="filters">
            <TextField id="search" size="small" label="Поиск" value={filters.search} onChange={() => onChange('search')} />
