...
import type { AccountCreate, AccountWithoutSpace } from "@/shared/types/Account";
    ...
    return (
        <UModal open={open} onClose={onClose} title="Создание нового счёта">
            <Box sx={{ p: '12px', minWidth: '920px', display: 'grid', gridTemplateColumns: '2fr 1fr 2fr', alignItems: 'center', gap: '12px' }}>
                <TextField required label="Наименование счёта" value={newAccount.name} size="small" onChange={(e) => handleChange('name', e.target.value)} />
            ...
            <Button sx={{ margin: '12px auto', width: '240px' }} loading={isPending} variant="contained" onClick={handleCreate}>Создать</Button>
        </UModal>
    )
}