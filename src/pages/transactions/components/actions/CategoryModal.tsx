                    ...
                    {spaceResp?.data.categories.map(cat => {
                        return <li className="category-modal__item" key={cat.id}>
                            ...
                            <span className="category-modal__actions">
                                <IconButton size='small' aria-label="редактировать" onClick={() => handleCategoryEdit(cat)}><EditIcon fontSize="inherit" /></IconButton>
                                <IconButton size='small' color="error" aria-label="удалить" onClick={() => handleCategoryDelete(cat)}><DeleteIcon fontSize="inherit" /></IconButton>
                            </span>
                        </li>
                    })}
                </ul>
                <Button sx={{ m: 'auto' }} color="secondary" onClick={() => openModal('create')}>Создать новую категорию</Button>

                <CreateCategory open={isOpen('create')} onClose={closeModal} />
                {editingCategory && <EditCategory open={isOpen('edit')} onClose={handleEditModalClose} category={editingCategory} />}
            </div>
        </UModal>
    )
}