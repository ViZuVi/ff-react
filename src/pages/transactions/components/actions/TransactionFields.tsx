    ...
    return (
        <div className="new-transaction-modal__fields">
            ...
            <FormControl size="small">
                ...
                >
                    {spaceResp?.data.accounts.map((acc) => <MenuItem value={acc.id}>{acc.name}</MenuItem>)}
                    <Button sx={{ m: 'auto', width: '100%' }} color="secondary" onClick={() => openModal('create-account')}>Создать новый</Button>
